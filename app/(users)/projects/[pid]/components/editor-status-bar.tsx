export const EditorStatusBar = ({ nodeId, initialContent = '' }: { nodeId: string; initialContent?: string }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 h-5 w-full bg-[#191d24] border-t border-white/5 flex items-center px-4 z-60 select-none">
      <div className="flex items-center gap-4 text-[10px] font-mono text-white/40 uppercase tracking-tight">
        <div className="flex gap-1.5">
          <span>Words:</span>
          {/* Unique ID: cm-word-count-123 */}
        ...
        <div className="flex gap-1.5">
          <span>Chars:</span>
          {/* Unique ID: cm-char-count-123 */}
          <span id={`cm-char-count-${nodeId}`} className="text-blue-400/80">
            {initialContent.length}
          </span>
        </div>
        <div className="ml-auto opacity-20">UTF-8</div>
      </div>
    </div>
  );
};

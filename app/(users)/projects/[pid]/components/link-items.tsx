...
const HighlightedLink = ({ text, searchQuery }: { text: string; searchQuery?: string }) => {
  ...
  return (
    <span className="text-[12px] leading-relaxed text-muted-foreground group-hover/mention:text-foreground transition-colors">
      {parts.map((part, i) => {
      ...
      <div className="group relative flex items-center gap-2 rounded cursor-pointer hover:bg-secondary transition-colors w-full" onClick={onHeaderClick}>
        ...
        <div className="flex flex-col w-full h-auto text-start py-1.5 overflow-hidden">
          <div className="flex items-center gap-2 pr-2">
            ...
            <span className="text-[13px] text-muted-foreground truncate font-medium tracking-tight">{file.title}</span>
          ...
          {file.mentions.map((mention, i: number) => (
            ...
            >
              ...
              <div className="flex flex-col gap-0.5 overflow-hidden">
                <span className="text-[9px] font-mono text-foreground uppercase">Line {mention.line}</span>

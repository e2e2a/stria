...
function RemoveMemberContent({ item }: { item: IWorkspaceMember }) {
  return (
    <AlertDialogDescription className="text-start">
      {item.user ? (
        <>
          This action cannot be undone. This will remove <span className="font-bold">{item.email}</span> from this workspace. They will immediately lose
        ...
        <>
          ...
          the workspace unless invited again.
        </>
      )}
    </AlertDialogDescription>
  );
...
export default function TrashDialog({ item, workspaceId }: IProps) {
  ...
  const [canTrash] = useState(item.status === 'accepted' ? !!mData?.permissions.canDeleteMember && !isLastOwner : !!mData?.permissions.canDeleteInvite);
        ...
        <div className="flex flex-col">
          <AlertDialogHeader className="gap-y-3">
            <AlertDialogTitle className="sm:text-xl text-start font-bold">
              Are you sure you want to delete the user{item.user ? '' : ' invitation'}?
            </AlertDialogTitle>
